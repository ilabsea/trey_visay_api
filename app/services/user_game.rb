...
class UserGame
  ...
  def setup_data(params)
    ...
    game_params.each do |game_param|
      game = @user.games.new(reason: game_param.reason)
      ...
      game_param.characteristic_entries.each do |entry|
        game.entries.new(entry)
      end

      # careers
      game_param.careers.each do |career|
        game.careers.new(career_id: career.id, is_goal: career.is_goal)
      end
    end
  end
...
#             },
#             characteristic_entries: ['អត់ធ្មត់', ...],
#             careers: [
